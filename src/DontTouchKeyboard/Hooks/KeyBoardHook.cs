namespace DontTouchKeyboard.Hooks;
...
internal class KeyBoardHook : IDisposable
{
    ...
    public KeyBoardHook(ProcessModule mainModule)
    {
        if (keyboardHandle.IsInvalid)
        {
            ...
            nint moduleHandle = Kernel32.GetModuleHandle(mainModule.ModuleName);
    ...
    private int KeyboardHook(int nCode, nint wParam, nint lParam)
    {
        if (nCode >= 0)
        {
